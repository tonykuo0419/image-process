...
public class BiLineRotateFilter extends AbstractBufferedImageOp {
    ...
    public BiLineRotateFilter() {
        System.out.println("run BiLineRotateFilter");
    ...
    public BufferedImage createCompatibleDestImage(BufferedImage src, ColorModel dstCM) {
        ...
        return new BufferedImage(dstCM, dstCM.createCompatibleWritableRaster(outw, outh),
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dest) {
        ...
        if (specialAngle > 0) { return specialAngleRotate(src, inPixels, width, height); }
    ...
    private BufferedImage specialAngleRotate(BufferedImage src, int[] inPixels, int width,
            int height) {
        int index = 0;
        int index2 = 0;
        if (specialAngle == 90) {
        ...
        for (int row = 0; row < height; row++) {
            ...
            for (int col = 0; col < width; col++) {
                ...
                if (specialAngle == 90) {
                    ...
                    index2 = outw * (height - 1 - row) + (width - 1 - col);
                } else if (specialAngle == 270) {
                    index2 = outw * (width - 1 - col) + row;
                }
                outPixels[index2] = (ta << 24) | (tr << 16) | (tg << 8) | tb;
            }
        }
    ...
    private int[] bilineInterpolation(int[] input, int width, int height, float prow, float pcol) {
        ...
        int tr = (int) (tr1 * coffiecent1 + tr2 * coffiecent4 + tr3 * coffiecent2
                + tr4 * coffiecent3);
        int tg = (int) (tg1 * coffiecent1 + tg2 * coffiecent4 + tg3 * coffiecent2
                + tg4 * coffiecent3);
        int tb = (int) (tb1 * coffiecent1 + tb2 * coffiecent4 + tb3 * coffiecent2
                + tb4 * coffiecent3);
