...
public class LanczosZoomFilter extends AbstractBufferedImageOp {
    ...
    public LanczosZoomFilter() {
        System.out.println("run LanczosZoomFilter");
        lanczosSize = 3;
        destWidth = 100;
    }
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dest) {
        ...
        if (dest == null) {
            ...
            dest = new BufferedImage(src.getColorModel(),
                    cMD.createCompatibleWritableRaster(dw, dh), cMD.isAlphaPremultiplied(), null);
        }
            ...
            for (int col = 0; col < dw; col++) {
                ...
                for (int subcol = (int) (icx - range2); subcol <= icx + range2; subcol++) {
                    ...
                    for (int subrow = (int) (icy - range2); subrow <= icy + range2; subrow++) {
                        ...
                        float weight = (float) getLanczosFactor(Math
                                .sqrt(Math.pow(ncol * rcp_ratio, 2) + Math.pow(nrow * rcp_ratio, 2))
                                / 1000);
                        if (weight > 0) {
