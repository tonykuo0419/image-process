 ...
 *
 */
public class HistogramEqualization4Gray extends AbstractBufferedImageOp {
    
    public HistogramEqualization4Gray() {
        System.out.println("run HistogramEqualization4Gray");
    }
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dest) {
        ...
        ArrayList<int[]> histLUT = histogramEqualizationLUT(src);

        BufferedImage histogramEQ = new BufferedImage(src.getWidth(), src.getHeight(),
                src.getType());
        ...
        for (int i = 0; i < src.getWidth(); i++) {
            for (int j = 0; j < src.getHeight(); j++) {
                ...
                // Get pixels by R, G, B
                alpha = new Color(src.getRGB(i, j)).getAlpha();
